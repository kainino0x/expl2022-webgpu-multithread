...
function establishStackSpace() {
 ...
 var stackTop = SAFE_HEAP_LOAD((pthread_ptr + 44 >> 2) * 4, 4, 0);
 var stackSize = SAFE_HEAP_LOAD((pthread_ptr + 48 >> 2) * 4, 4, 0);
...
var WebGPU = {
 ...
 initManagers: function() {
  ...
  WebGPU.mgrDevice = sharedManager;
  WebGPU.mgrQueue = WebGPU.mgrQueue || new Manager();
  WebGPU.mgrCommandBuffer = sharedManager;
  ...
  WebGPU.mgrBindGroup = sharedManager;
  WebGPU.mgrBuffer = sharedManager;
  WebGPU.mgrSampler = WebGPU.mgrSampler || new Manager();
  WebGPU.mgrTexture = WebGPU.mgrTexture || new Manager();
  WebGPU.mgrTextureView = sharedManager;
  ...
  WebGPU.mgrRenderPipeline = sharedManager;
  WebGPU.mgrComputePipeline = sharedManager;
  ...
  WebGPU.mgrRenderBundle = sharedManager;
 },
 ...
 function makeRenderBundleEncoderDescriptor(descriptor) {
  ...
  function makeColorFormats(count, formatsPtr) {
   ...
   for (var i = 0; i < count; ++i, formatsPtr += 4) {
    formats.push(WebGPU.TextureFormat[SAFE_HEAP_LOAD((formatsPtr >> 2) * 4, 4, 1)]);
   }
   return formats;
  }
  var desc = {
   "label": undefined,
   "colorFormats": makeColorFormats(SAFE_HEAP_LOAD((descriptor + 8 >> 2) * 4, 4, 1), SAFE_HEAP_LOAD((descriptor + 12 >> 2) * 4, 4, 1)),
   "depthStencilFormat": WebGPU.TextureFormat[SAFE_HEAP_LOAD((descriptor + 16 >> 2) * 4, 4, 1)],
   "sampleCount": SAFE_HEAP_LOAD((descriptor + 20 >> 2) * 4, 4, 1),
   "depthReadOnly": SAFE_HEAP_LOAD(descriptor + 24 >> 0, 1, 0) !== 0,
   "stencilReadOnly": SAFE_HEAP_LOAD(descriptor + 25 >> 0, 1, 0) !== 0
  };
  var labelPtr = SAFE_HEAP_LOAD((descriptor + 4 >> 2) * 4, 4, 1);
  if (labelPtr) desc["label"] = UTF8ToString(labelPtr);
  return desc;
...
function _wgpuQuerySetReference(id) {
 WebGPU.mgrQuerySet.reference(id);
}
...
var asmLibraryArg = {
 ...
 "wgpuQuerySetReference": _wgpuQuerySetReference,
